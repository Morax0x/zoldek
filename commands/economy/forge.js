...
const safeQuery = async (db, qPg, params) => {
    try { 
        ...
        // التوافق مع مشكلة (Case Sensitivity) للأسماء في الداتا بيز
        let fallbackQuery = qPg;
        if(qPg.includes('"userID"')) fallbackQuery = fallbackQuery.replace(/"userID"/g, "userid");
        if(qPg.includes('"guildID"')) fallbackQuery = fallbackQuery.replace(/"guildID"/g, "guildid");
        if(qPg.includes('"itemID"')) fallbackQuery = fallbackQuery.replace(/"itemID"/g, "itemid");
        if(qPg.includes('"skillID"')) fallbackQuery = fallbackQuery.replace(/"skillID"/g, "skillid");
        if(qPg.includes('"skillLevel"')) fallbackQuery = fallbackQuery.replace(/"skillLevel"/g, "skilllevel");
        if(qPg.includes('"raceName"')) fallbackQuery = fallbackQuery.replace(/"raceName"/g, "racename");
        if(qPg.includes('"weaponLevel"')) fallbackQuery = fallbackQuery.replace(/"weaponLevel"/g, "weaponlevel");
