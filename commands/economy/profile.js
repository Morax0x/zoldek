...
// 🔥 جلب الدوال الذكية من ملف الرسام 🔥
let resolveItemInfoLocal, getInventoryCategoriesLocal;
try {
    const invGen = require('../generators/inventory-generator.js');
    resolveItemInfoLocal = invGen.resolveItemInfo;
    getInventoryCategoriesLocal = invGen.getInventoryCategories; // الدالة اللي ترتب الصيد والموارد
} catch (e) {
    resolveItemInfoLocal = function(itemId) {
        return { name: itemId, emoji: '📦', category: 'others', rarity: 'Common', imgPath: null, description: "تعذر قراءة التفاصيل" };
    };
    getInventoryCategoriesLocal = null;
}
...
module.exports = {
    ...
    async execute(interactionOrMessage, args) {
        ...
        try {
            ...
            let invCategory = 'materials'; 
            let invPage = 1; 
            let skillPage = 0;
            let selectedIndex = 0; 
            let activeItemDetails = null; 

            // 🔥 حل مشكلة الاختصارات بدون بريفكس (للكازينو) 🔥
            let commandTrigger = "";
            if (!isSlash) {
                const contentParts = interactionOrMessage.content.trim().split(/ +/);
                const firstWord = contentParts[0].toLowerCase();
                const aliases = ['p', 'بروفايل', 'بطاقة', 'كارد', 'card', 'inv', 'inventory', 'شنطة', 'اغراض', 'حقيبة', 'مهاراتي', 'skills', 'ms', 'عتاد', 'قدراتي', 'محفظتي', 'استثماراتي', 'ممتلكات', 'portfolio'];
                
                // قراءة الكلمة سواء كانت ببريفكس أو بدونه!
                if (aliases.includes(firstWord)) {
                    commandTrigger = firstWord; 
                } else if (firstWord.length > 1 && aliases.includes(firstWord.slice(1))) {
                    commandTrigger = firstWord.slice(1);
                }
            }
            ...
            const renderView = async () => {
                ...
                if (currentView === 'inventory') {
                    ...
                    // 🔥 باقي أقسام الحقيبة العادية (مربوطة بالدالة المركزية لترتيب الصيد والموارد) 🔥
                    else {
                        try {
                            if (getInventoryCategoriesLocal) {
                                const allCategories = await getInventoryCategoriesLocal(db, targetUser.id, guildId);
                                items = allCategories[invCategory] || [];
                            } else {
            ...
            collector.on('collect', async (i) => {
                ...
                // 🔥 توحيد أسماء الأقسام للإنجليزية لضمان التطابق مع قاموس الرسام 🔥
