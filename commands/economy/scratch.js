...
const RACE_ASSETS = {
    ...
    'Default':  { bg: 'card.png',        frame: null } 
};

const RACE_COLORS = {
    'Human':    'rgba(100, 110, 120, 0.35)', 
    'Dragon':   'rgba(212, 175, 55, 0.35)',  
    ...
    async execute(interactionOrMessage, args) {
        ...
        try {
            ...
            const balancePromise = (async () => {
                let sMora = 0, sBank = 0;
                try {
                    let res = await client.sql.query(`SELECT "mora", "bank" FROM levels WHERE "user" = $1 AND "guild" = $2`, [user.id, guild.id]);
                    if (res && res.rows && res.rows.length > 0) {
                        sMora = Number(res.rows[0].mora) || 0;
                        sBank = Number(res.rows[0].bank) || 0;
                        return { sMora, sBank };
                    }
                } catch (e1) {
                    try {
                        let res = await client.sql.query(`SELECT mora, bank FROM levels WHERE userid = $1 AND guildid = $2`, [user.id, guild.id]);
                        if (res && res.rows && res.rows.length > 0) {
                            sMora = Number(res.rows[0].mora) || 0;
                            sBank = Number(res.rows[0].bank) || 0;
                            return { sMora, sBank };
                ...
                if (client.getLevel && sMora === 0 && sBank === 0) {
                    try {
