...
// =========================================================================
// 🛡️ دوال حماية خاصة بجدول المشاركين لضمان الإضافة مهما كان اسم العمود 🛡️
// =========================================================================
async function getGiveawayEntry(db, messageID, userID) {
    let res;
    try { res = await db.query('SELECT * FROM giveaway_entries WHERE "giveawayID" = $1 AND "userID" = $2', [messageID, userID]); if(res.rows.length) return res.rows[0]; } catch(e){}
    try { res = await db.query('SELECT * FROM giveaway_entries WHERE giveawayid = $1 AND userid = $2', [messageID, userID]); if(res.rows.length) return res.rows[0]; } catch(e){}
    try { res = await db.query('SELECT * FROM giveaway_entries WHERE "messageID" = $1 AND "userID" = $2', [messageID, userID]); if(res.rows.length) return res.rows[0]; } catch(e){}
    try { res = await db.query('SELECT * FROM giveaway_entries WHERE messageid = $1 AND userid = $2', [messageID, userID]); if(res.rows.length) return res.rows[0]; } catch(e){}
    return null;
}

async function insertGiveawayEntry(db, messageID, userID, weight) {
    try { await db.query('INSERT INTO giveaway_entries ("giveawayID", "userID", "weight") VALUES ($1, $2, $3)', [messageID, userID, weight]); return true; } catch(e){}
    try { await db.query('INSERT INTO giveaway_entries (giveawayid, userid, weight) VALUES ($1, $2, $3)', [messageID, userID, weight]); return true; } catch(e){}
    try { await db.query('INSERT INTO giveaway_entries ("messageID", "userID", "weight") VALUES ($1, $2, $3)', [messageID, userID, weight]); return true; } catch(e){}
    try { await db.query('INSERT INTO giveaway_entries (messageid, userid, weight) VALUES ($1, $2, $3)', [messageID, userID, weight]); return true; } catch(e){}
    return false;
}

async function deleteGiveawayEntry(db, messageID, userID) {
    try { await db.query('DELETE FROM giveaway_entries WHERE "giveawayID" = $1 AND "userID" = $2', [messageID, userID]); return true; } catch(e){}
    try { await db.query('DELETE FROM giveaway_entries WHERE giveawayid = $1 AND userid = $2', [messageID, userID]); return true; } catch(e){}
    try { await db.query('DELETE FROM giveaway_entries WHERE "messageID" = $1 AND "userID" = $2', [messageID, userID]); return true; } catch(e){}
    try { await db.query('DELETE FROM giveaway_entries WHERE messageid = $1 AND userid = $2', [messageID, userID]); return true; } catch(e){}
    return false;
}

async function countGiveawayEntries(db, messageID) {
    let res;
    try { res = await db.query('SELECT COUNT(*) as count FROM giveaway_entries WHERE "giveawayID" = $1', [messageID]); return parseInt(res.rows[0]?.count || res.rows[0]?.COUNT || 0, 10); } catch(e){}
    try { res = await db.query('SELECT COUNT(*) as count FROM giveaway_entries WHERE giveawayid = $1', [messageID]); return parseInt(res.rows[0]?.count || res.rows[0]?.COUNT || 0, 10); } catch(e){}
    try { res = await db.query('SELECT COUNT(*) as count FROM giveaway_entries WHERE "messageID" = $1', [messageID]); return parseInt(res.rows[0]?.count || res.rows[0]?.COUNT || 0, 10); } catch(e){}
    try { res = await db.query('SELECT COUNT(*) as count FROM giveaway_entries WHERE messageid = $1', [messageID]); return parseInt(res.rows[0]?.count || res.rows[0]?.COUNT || 0, 10); } catch(e){}
    return 0;
}

async function getAllGiveawayEntries(db, messageID) {
    let res;
    try { res = await db.query('SELECT "userID" as userid, "weight" FROM giveaway_entries WHERE "giveawayID" = $1', [messageID]); return res.rows; } catch(e){}
    try { res = await db.query('SELECT userid, weight FROM giveaway_entries WHERE giveawayid = $1', [messageID]); return res.rows; } catch(e){}
    try { res = await db.query('SELECT "userID" as userid, "weight" FROM giveaway_entries WHERE "messageID" = $1', [messageID]); return res.rows; } catch(e){}
    try { res = await db.query('SELECT userid, weight FROM giveaway_entries WHERE messageid = $1', [messageID]); return res.rows; } catch(e){}
    return [];
}
// =========================================================================
...
// 🛠️ إصلاح التفاعل والرد باستخدام دوال الحماية الجديدة 🛠️
async function handleGiveawayInteraction(client, interaction) {
    try {
        ...
        // 🔥 إرسال رد مبدئي مخفي لكي لا يعلق الزر 🔥
        ...
        const existingEntry = await getGiveawayEntry(db, messageID, userID);
        let replyMessage = "";
        ...
        // تحديث الزر والإيمبد بعدد المشاركين
        try {
            const count = await countGiveawayEntries(db, messageID);

            const embed = EmbedBuilder.from(interaction.message.embeds[0]);
            
            // تحديث الرقم في الوصف الجميل الخاص بك
...
async function endGiveaway(client, messageID, force = false) {
    try {
        ...
        const entries = await getAllGiveawayEntries(db, messageID);
