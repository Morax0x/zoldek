        ...
        if (battleState.isPvE) {
            if (winnerId !== "monster") {
                ...
                if (addXPAndCheckLevel && winner.member) {
                    ...
                    const userDataRes = await db.query(`SELECT * FROM levels WHERE "user" = $1 AND "guild" = $2`, [winner.member.id, guildId]).catch(()=>({rows:[]}));
                    let userData = userDataRes.rows[0];
                    if(userData) {
                        userData.mora = Number(userData.mora) + rewardMora;
                        userData.xp = Number(userData.xp) + rewardXP;
                        await db.query(`UPDATE levels SET "mora" = $1, "xp" = $2 WHERE "user" = $3 AND "guild" = $4`, [userData.mora, userData.xp, winner.member.id, guildId]).catch(()=>{});
                    }
                }
            ...
            if (cancelRewards) {
                ...
                if (rankPoint && !winner.isBot && winner.member && (settings.rolePvPKing || settings.rolepvpking) && winner.member.roles.cache.has(settings.rolePvPKing || settings.rolepvpking)) {
                    const stealAmount = Math.floor((battleState.bet || 0) * 0.10);
                    
                    if (!loser.isBot && loser.member) {
                        const loserDataRes = await db.query(`SELECT "mora", "bank" FROM levels WHERE "user" = $1 AND "guild" = $2`, [loserId, guildId]).catch(()=>({rows:[]}));
                        let loserData = loserDataRes.rows[0] || { mora: 0, bank: 0 };
                        let lMora = Number(loserData.mora);
                        let lBank = Number(loserData.bank);

                        if (lMora >= stealAmount) lMora -= stealAmount;
                        else if (lBank >= stealAmount) lBank -= stealAmount;
                        else lMora = 0; 
                        
                        await db.query(`UPDATE levels SET "mora" = $1, "bank" = $2 WHERE "user" = $3 AND "guild" = $4`, [lMora, lBank, loserId, guildId]).catch(()=>{});
                        if (discordClient?.updateLevelField) discordClient.updateLevelField(loserId, guildId, { mora: lMora, bank: lBank });
                    }
                ...
                if (!winner.isBot && winner.member && (settings.roleCasinoKing || settings.rolecasinoking) && !winner.member.roles.cache.has(settings.roleCasinoKing || settings.rolecasinoking)) {
                    ...
                    if (kingMembers && kingMembers.size > 0) {
                        ...
                        if (casinoTax > 0) {
                            ...
                            await db.query(`UPDATE levels SET "bank" = COALESCE(CAST("bank" AS BIGINT), 0) + $1 WHERE "user" = $2 AND "guild" = $3`, [casinoTax, king.id, guildId]).catch(()=>{});
                        }
                    }
                }
                ...
                if (!winner.isBot && winner.member) {
                    ...
                    if (updateGuildStat) updateGuildStat(discordClient, guildId, winnerId, 'pvp_wins', 1).catch(()=>{});
                    ...
                    await db.query(`INSERT INTO user_buffs ("guildID", "userID", "buffPercent", "expiresAt", "buffType", "multiplier") VALUES ($1, $2, $3, $4, $5, $6)`, [guildId, winnerId, 15, expireTime, 'mora', 0.15]).catch(()=>{});
                    await db.query(`INSERT INTO user_buffs ("guildID", "userID", "buffPercent", "expiresAt", "buffType", "multiplier") VALUES ($1, $2, $3, $4, $5, $6)`, [guildId, winnerId, 15, expireTime, 'xp', 0.15]).catch(()=>{});
                }
