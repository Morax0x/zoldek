...
function buildPvpSkillSelector(battleState) {
    try {
        ...
        const availableSkills = Object.values(userSkills).filter(s => s && (s.currentLevel > 0 || (s.id && s.id.startsWith('race_'))));
        ...
        availableSkills.forEach(skill => {
            ...
            const option = new StringSelectMenuOptionBuilder()
                .setLabel(String(skill.name).substring(0, 100))
                .setValue(String(skill.id).substring(0, 100))
                .setDescription(String(cdText).substring(0, 100));

            if (skill.emoji) {
                try { option.setEmoji(skill.emoji); } catch (e) { option.setEmoji('✨'); }
            } else {
                option.setEmoji('✨');
            }
        ...
        console.error("[PvP Skill Selector Error]:", e);
        return null;
    }
}
...
async function buildBattleEmbed(battleState) {
    try {
        ...
        if (!attacker.isMonster && !attacker.isBot) {
            ...
            // 🔥 إخفاء زر الانسحاب إذا كانت معركة PvE (ضد وحش) أو ضد الفارس 🔥
        ...
        if (files.length === 0) {
            ...
            if (battleState.isGuardBattle) {
                ...
                embed.setDescription(`الرهان: **${((battleState.bet || 0) * 2).toLocaleString()}** ${EMOJI_MORA}\n\n**الدور الآن لـ:** ${turnMention}`);
            }
        ...
        console.error("[PvP UI Error in buildBattleEmbed]:", e);
        return { embeds: [], components: [], files: [] };
    }
}
...
async function updateSpectatorEmbed(battleState) {
    try {
        ...
        const p1Name = String(p1?.isBot ? p1.name : cleanDisplayName(p1?.member?.displayName || p1?.member?.user?.username || 'اللاعب 1')).substring(0, 40);
        const p2Name = String(p2?.isBot ? p2.name : cleanDisplayName(p2?.member?.displayName || p2?.member?.user?.username || 'اللاعب 2')).substring(0, 40);
        ...
        console.error("[Spectator Embed Edit Error]:", e);
    }
}
