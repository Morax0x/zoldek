...
function calculateCP(player) {
    let cp = (player.maxHp * 0.5) + ((player.weapon?.currentDamage || 0) * 10);
    let skillScore = 0;
    if (player.skills) {
...
async function startPvpBattle(i, client, db, challengerMember, opponentMember, bet, isBotMatch = false) {
    ...
    const opponentName = isBotMatch ? "الزعيم موركس" : cleanDisplayName(opponentMember.user.displayName);
    ...
    threadCollector.on('collect', async (msg) => {
        ...
        if (messageCounter >= 20 && !bumpCooldown) {
            ...
            try {
                ...
                if (battleState.message && battleState.message.deletable) {
                    await battleState.message.delete().catch(() => {});
                }
            } catch (e) {}
...
async function endBattle(battleState, winnerId, db, reason = "win", buffCalculator = null) {
    ...
    if (battleState.isPvE) {
        ...
        if (winnerId !== "monster") {
            ...
            if (addXPAndCheckLevel) await addXPAndCheckLevel(battleState.message.client, winner.member, db, rewardXP, rewardMora, false).catch(()=>{});
            else await db.query(`UPDATE levels SET "mora" = "mora" + $1, "xp" = "xp" + $2 WHERE "user" = $3 AND "guild" = $4`, [rewardMora, rewardXP, winner.member.id, guildId]).catch(()=>{});
            ...
            embed.setColor(Colors.Gold).setThumbnail(winner.member.displayAvatarURL()).setImage(WIN_IMAGES[Math.floor(Math.random() * WIN_IMAGES.length)])
    ...
    const rounds = Math.floor(stats.actions / 2);
    ...
    const cpDiff = Math.abs(winnerCP - loserCP) / Math.max(winnerCP, loserCP);
    if (cpDiff > 0.4) score -= 25;
    else if (cpDiff > 0.2) score -= 10;

    const loserDamage = stats[loserId].damageDealt;
    const damageRatio = loserDamage / winner.maxHp;
    if (damageRatio < 0.15) score -= 40;
    else if (damageRatio < 0.30) score -= 15;

    const loserSkillsCount = Object.keys(loser.skills || {}).length;
    if (stats[loserId].skillsUsed === 0 && loserSkillsCount > 0) score -= 15;
    ...
    if (cancelRewards) {
        ...
        if (!winner.isBot && settings.roleCasinoKing && !winner.member.roles.cache.has(settings.roleCasinoKing)) {
            ...
            if (kingMembers && kingMembers.size > 0) {
                ...
                if (casinoTax > 0) {
                    ...
                    await db.query(`UPDATE levels SET "bank" = "bank" + $1 WHERE "user" = $2 AND "guild" = $3`, [casinoTax, kingMembers.first().id, guildId]).catch(()=>{});
                }
            }
        }
    ...
    try {
        ...
        await battleState.message.channel.send({ content: `**انتهت المعركة!** التقييم: ${gradeText} | الفائز ربح ${finalWinnings} مورا.` });
    }
