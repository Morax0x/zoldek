            ...
            if (i.customId === 'retreat') {
                ...
                if (p && p.class === 'Leader') {
                    ...
                    if (pIndex > -1) {
                        ...
                        for(let x = startFloor; x <= floor; x++) { 
                            let rReward = repM[x] || 0;
                            let cReward = rReward + (x === 5 || x === 10 ? 1 : 0);
                            pRepReward += rReward;
                            pChests += cReward; 
                        }
                        ...
                        await threadChannel.send(`💨 **${leavingPlayer.name}** انسـحـب من ارض المعـركـة!`).catch(()=>{});
