...
async function generateDailyQuestsImage(member, questsData, page = 1) {
    try {
        ...
        ctx.textAlign = 'right';
        ctx.textBaseline = 'middle';
        ctx.fillText(`المهـام اليـوميـة لـ ${mName}`, PAGE_WIDTH - PAGE_MARGIN, headerY);
        ...
        const attachment = new AttachmentBuilder(await canvas.encode('png'), { name: `daily-quests-${member?.id || 'user'}-p${page}.png` });

        return { attachment, totalPages };
    } catch (err) {
        console.error("[generateDailyQuestsImage Error]:", err);
        throw err;
    }
}
