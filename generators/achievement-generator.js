...
async function generateAchievementPageImage(member, achievementsData, stats) {
    ...
    return new AttachmentBuilder(await canvas.encode ? canvas.encode('image/png') : Promise.resolve(canvas.toBuffer('image/png')), { name: `achievements-page-${member.id}-${stats.page}.png` });
}
...
async function generateSingleAchievementAlert(member, achievement) {
    ...
    return new AttachmentBuilder(await canvas.encode ? canvas.encode('image/png') : Promise.resolve(canvas.toBuffer('image/png')), { name: `achievement-unlocked-${member.id}-${achievement.id}.png` });
}
...
async function generateQuestAlert(member, quest, questType) {
    ...
    return new AttachmentBuilder(await canvas.encode ? canvas.encode('image/png') : Promise.resolve(canvas.toBuffer('image/png')), { name: `quest-unlocked-${member.id}-${quest.id}.png` });
}
